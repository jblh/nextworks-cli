...
export async function addBlocks(options: AddBlocksOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (await fileExists("app/layout.tsx")) {
        ...
        const { upgradeLayout } = await promptFn([
          {
            ...
            name: "upgradeLayout",
            message:
              "Would you like the CLI to automatically wrap your app/layout.tsx with AppProviders and add suppressHydrationWarning to the <html> tag?",
            default: true,
          },
        ]);
    ...
    if (!layoutUpgraded) {
      console.log(
        "1. Wrap your app with the AppProviders wrapper in app/layout.tsx (or your root layout):",
      );
      ...
      console.log(
        "1. app/layout.tsx was updated to wrap the app with AppProviders and add suppressHydrationWarning.",
      );
      console.log(
        "2. Ensure Tailwind is configured and app/globals.css is present (copied if available).",
      );
