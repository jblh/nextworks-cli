...
export function BrandNodeIconBadge({
  ...
  const base =
    "grid h-8 w-8 place-items-center rounded-md bg-primary/10 ring-1 ring-primary/30 text-primary";
  return (
...
export function BrandNodeGeometricMark({ className }: BrandNodeProps) {
  const base =
    "grid h-8 w-8 place-items-center rounded-md bg-gradient-to-br from-primary/10 to-accent/10 ring-1 ring-border";
  return (
...
export function BrandNodeEmojiBadge({
  className,
  emoji = "⚡",
}: BrandNodeEmojiBadgeProps) {
  const base =
    "grid h-8 w-8 place-items-center rounded-md bg-card/60 backdrop-blur ring-1 ring-border";
  return (
...
export function BrandNodeDiagonalAppIcon({
  ...
  const base =
    "relative h-8 w-8 overflow-hidden rounded-md ring-1 ring-black/5 dark:ring-white/10";
  return (
    <div className={cn(base, className)} aria-label="Brand diagonal app icon">
      <div className={cn("absolute inset-0", baseColorClass)} />
      <div
        className={cn(
          "absolute inset-0 translate-y-1/3 -skew-y-12",
          overlayColorClass,
        )}
      />
    </div>
...
export function BrandNodeCubeOutline({ className }: BrandNodeProps) {
  const base =
    "grid h-8 w-8 place-items-center rounded-md bg-foreground text-background ring-1 ring-border";
  return (
    <div className={cn(base, className)} aria-label="Brand cube outline">
      <svg viewBox="0 0 24 24" className="h-4 w-4">
        <path
          d="M12 2l8 4.5v9L12 20 4 15.5v-9L12 2z"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.6"
        />
        <path
          d="M12 20v-9M4 6l8 5 8-5"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.6"
        />
