...
1. Check $ARGUMENTS (after "load"):
   - If it looks like a filename (single word, no spaces): Look for `.continue/specs/{name}.md`
...
5. Update current-feature.md:
   ...
   - Write any additional notes/context under ## Notes
   - Set Status to "Not Started"
6. Confirm spec loaded and show the feature summary
...
### If action is "load, start, review"

1. Treat the argument as a chained workflow and perform load, then start, then review in sequence.
2. Do not prompt for confirmation between steps.
3. Continue automatically until all three steps finish or a blocking error occurs.
4. After load completes, immediately run start.
5. After start completes, immediately run review.
6. Return one combined result for the full workflow.
